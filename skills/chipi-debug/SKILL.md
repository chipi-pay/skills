...
| "WALLET_NOT_COMPATIBLE" | Attempting session keys on READY wallet | Check `wallet.walletType`. READY wallets don't support sessions. | Upgrade to CHIPI using `sdk.prepareWalletUpgrade()` + `sdk.executeWalletUpgrade()` (backend-only, requires secret key). |
